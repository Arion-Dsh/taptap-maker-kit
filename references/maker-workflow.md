...
主流程：游戏设计 → Maker 初始化 → 开发 → 发布预览。测试二维码是按需操作，只有用户明确要求生成时才执行。
...
图标、宣传图、截图、实机录屏和文案位于外层 `publish/`。制作或验收时读取[发布素材](publish-assets.md)；不要为了准备商店资料而触发 `game/` 预览或推送。

## 发布预览

仅在用户明确要求构建、预览、提交或推送时，从 `game/` 调用 `maker_build_current_directory` 并跟随 `next_action`。由 Maker 流程同步、合并并推送内层仓库，不自行假定具体 Git 步骤。

预览成功后，从工具结果中提取预览 URL，并以可点击链接返回给用户。不得主动打开预览页、浏览器或其他应用；即使 `next_action` 建议打开预览，也只返回链接。只有用户明确要求打开时才执行打开操作。

冲突、认证失败或远端拒绝时保留证据并停止，不丢弃改动或强推。内层推送成功后才更新外层 submodule 指针。外层 `publish/` 只是商店宣发素材目录。

## 测试二维码

不得因预览、构建、提交或推送成功而主动生成测试二维码。只有用户明确要求生成测试二维码时，才在预览成功后从 `game/` 上下文调用 `generate_test_qrcode`。使用工具返回的二维码及构建信息，不在外层自行生成。
