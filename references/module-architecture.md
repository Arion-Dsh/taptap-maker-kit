...
HUD、按钮、弹窗、教程、提示、状态、错误和空页面等玩家可见文案必须使用玩家视角，并符合当前游戏的世界观、角色口吻和玩法语境。优先沿用项目已有的名词、语气和表达；没有明确设定时，用简短自然的游戏语言说明玩家当前能感知的状态、可执行的动作或获得的结果。

不得把研发或策划过程暴露到正式游戏 UI，包括“功能开发中”“策划配置”“测试入口”“数值待调”“占位文案”“配置缺失”“接口报错”“服务端返回”等内部工作语气。运行失败时，把内部原因转换成玩家能理解且可行动的提示，例如说明暂时不可用、稍后重试或返回上一页；错误码、字段名、堆栈和配置状态只写入日志或与正式 UI 隔离的开发调试界面。

修改 UI 时同时检查新增和受影响的现有文案。不要仅把技术词换成口语，还要确认措辞像游戏本身在与玩家交流，而不是开发团队在解释实现状态。
